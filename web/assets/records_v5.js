...
(function () {
  ...
  // 后端：Fntv-net 项目的 Cloudflare Worker（默认 https://690075.xyz）：
  ...
  function buildLevelModal() {
    ...
    wrap.innerHTML = [
      ...
      '            <label>排行榜服务器<input type="text" id="dzk-lb-api" placeholder="https://690075.xyz"></label>',
      '            <div class="dzk-lb-hint">成绩 = 无尽层数，只有打破个人纪录才会上传。默认走 Fntv-net 服务器，拉不到时自动显示本机战绩。</div>',
    ...
    if (lbGear && lbSettings) {
      ...
      var lbApiInput = wrap.querySelector('#dzk-lb-api');
      lbGear.addEventListener('click', function () {
        var c = lbCfg();
        lbNameInput.value = (c.name && String(c.name).trim()) || '';
        lbApiInput.value = (c.api && String(c.api).trim()) || '';
