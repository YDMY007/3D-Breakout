...
(function () {
  ...
  function genEndlessLayer(floor) {
    ...
    // 专家档(4)之后，层数越高密度再略微提升，但有上限，保证可玩
    ...
    var nLayers = layMin + Math.floor(rng() * (layMax - layMin + 1));
    var grid = [];
    for (var r = 0; r < BN; r++) { grid[r] = []; for (var c = 0; c < PC; c++) grid[r][c] = [0, 0, 0, 0]; }
    var totalCells = PC * BN, baseDensity = fillMin + rng() * (fillMax - fillMin);
    var baseTarget = Math.floor(totalCells * baseDensity);
    var cols = [];
    for (c = 0; c < PC; c++) cols.push(c);
    for (var i = cols.length - 1; i > 0; i--) { var j = Math.floor(rng() * (i + 1)); var tmp = cols[i]; cols[i] = cols[j]; cols[j] = tmp; }
    var nCols = Math.max(3, Math.floor(PC * baseDensity));
    var support = cols.slice(0, nCols);
    for (var si = 0; si < support.length; si++) {
      var col = support[si];
      var maxH = 1 + Math.floor(rng() * nLayers);
      for (var h = 0; h < maxH; h++) for (var rr = 0; rr < BN; rr++) grid[rr][col][h] = endlessWeightedType(conf.weights, rng);
    }
    var filled = 0, r2, cc, hh;
    for (r2 = 0; r2 < BN; r2++) for (cc = 0; cc < PC; cc++) for (hh = 0; hh < QS; hh++) if (grid[r2][cc][hh]) filled++;
    var targetTotal = Math.floor(totalCells * (fillMin + rng() * (fillMax - fillMin)));
    var attempts = 0;
    while (filled < targetTotal && attempts < 400) {
      attempts++;
      var r0 = Math.floor(rng() * BN), c0 = Math.floor(rng() * PC);
      var top = -1;
      for (hh = 0; hh < QS; hh++) if (grid[r0][c0][hh]) top = hh;
      if (top >= 0) {
        if (top + 1 < QS && rng() < 0.6) { grid[r0][c0][top + 1] = endlessWeightedType(conf.weights, rng); filled++; }
        else {
          var nr = r0 + (rng() < 0.5 ? -1 : 1), nc = c0 + (rng() < 0.5 ? -1 : 1);
          if (nr >= 0 && nr < BN && nc >= 0 && nc < PC && !grid[nr][nc][top] && (top === 0 || grid[nr][nc][top - 1])) {
            grid[nr][nc][top] = endlessWeightedType(conf.weights, rng); filled++;
          }
        }
      } else if (rng() < 0.5) { grid[r0][c0][0] = endlessWeightedType(conf.weights, rng); filled++; }
    }
  ...
  // ===== 全球排行榜（无尽层数）=====
  // 成绩 = 单局无尽到达的层数。本局结束（层被重置为 1）时自动上报。
  // 接入方式（排行榜设置里填）：Dreamlo 免费排行榜（dreamlo.com 注册拿 pub/priv 码），
  // 或自定义 REST：dzk3d_lb 里直接写 get / post 模板（{name} {score} 占位）。
  // 未配置时自动降级为本机战绩榜，随时可接入。
  var LB_KEY = 'dzk3d_lb';          // {name, pub, priv} 或 {name, get, post}
  ...
  function lbName() {
    var c = lbCfg();
    return (c.name && String(c.name).trim()) || '无名玩家';
  }
  ...
  function lbAddLocal(floor) {
    var list = lbLocalList();
    list.push({ name: lbName(), floor: floor, date: Date.now() });
  ...
  function lbUrls() {
    ...
    if (c.pub && c.priv) {
      return {
        get: 'https://dreamlo.com/lb/' + encodeURIComponent(c.pub) + '/json',
        post: function (name, floor) {
          return 'https://dreamlo.com/lb/' + encodeURIComponent(c.priv) + '/add/' +
            encodeURIComponent(name) + '/' + encodeURIComponent(floor);
        }
      };
    }
    if (c.get || c.post) {
      return {
        get: c.get || '',
        post: c.post ? function (name, floor) {
          return c.post.replace('{name}', encodeURIComponent(name)).replace('{score}', encodeURIComponent(floor));
        } : null
      };
    }
    return null;
  }
  function lbSubmit(floor) {
    var urls = lbUrls();
    if (!urls || !urls.post || !(floor > 0)) return;
    try {
      var url = urls.post(lbName(), floor);
      if (window.fetch) fetch(url, { mode: 'no-cors' }).catch(function () {});
      else new Image().src = url;
    } catch (_) {}
  }
  function lbParseRemote(jsonText) {
    var rows = [];
    var o = JSON.parse(jsonText);
    // Dreamlo 格式: {dreamlo:{leaderboard:{player:[{name,score}]}}}
    var players = o && o.dreamlo && o.dreamlo.leaderboard && o.dreamlo.leaderboard.player;
    if (!players && Array.isArray(o)) players = o; // 通用格式: [{name, floor|score}]
    if (Array.isArray(players)) {
      players.forEach(function (p) {
        rows.push({
          name: String(p.name || p.player || '?'),
          floor: Number(p.floor != null ? p.floor : p.score) || 0
        });
      });
    }
    rows.sort(function (a, b) { return b.floor - a.floor; });
    return rows.slice(0, 20);
  }
  function lbFetchRemote(done) {
    var urls = lbUrls();
    if (!urls || !urls.get) { done(null); return; }
    try {
      var xhr = new XMLHttpRequest();
      xhr.timeout = 4000;
      xhr.open('GET', urls.get + (urls.get.indexOf('?') > -1 ? '&' : '?') + '_=' + Date.now());
      xhr.onload = function () {
        var rows = null;
        try { rows = lbParseRemote(xhr.responseText); } catch (_) {}
        if (rows && rows.length) {
  ...
  function lbRenderRows(listEl, rows, mineName) {
    var html = '';
    if (!rows || !rows.length) {
      html = '<div class="dzk-lb-empty">暂无战绩</div>';
    } else {
      rows.slice(0, 10).forEach(function (r, idx) {
        ...
        html += '<div class="dzk-lb-row' + me + '">' +
          ...
          '<span class="dzk-lb-score">' + r.floor + ' 层</span></div>';
      });
    }
    listEl.innerHTML = html;
  }
  function renderLeaderboard() {
    ...
    if (!listEl) return;
    var mineName = lbName();
    var myBest = Number(localStorage.getItem('dzk3d_endless_best')) || 0;
    if (mineEl) mineEl.textContent = '我: ' + (myBest > 0 ? myBest + ' 层' : '—');
    var urls = lbUrls();
    if (!urls) {
      // 离线模式：本机战绩榜
      var local = lbLocalList().slice(0, 10);
      lbRenderRows(listEl, local, mineName);
      return;
    }
    // 联网模式：先画缓存，再拉远端刷新
    var cache = null;
    ...
    if (cache && cache.length) {
      lbRenderRows(listEl, cache, mineName);
      lbFetchRemote(function (rows) { if (rows) lbRenderRows(listEl, rows, mineName); });
    } else {
      lbRenderRows(listEl, null, mineName);
      var local = lbLocalList().slice(0, 10);
      if (local.length) lbRenderRows(listEl, local, mineName);
      lbFetchRemote(function (rows) { if (rows) lbRenderRows(listEl, rows, mineName); });
    }
  }
    ...
    Object.defineProperty(window, '__dzkEndlessFloor', {
      ...
      set: function (v) {
        ...
        try {
          ...
          } else if (v === 1 && __lbSession > 0) {
            // 单局结束（层数被重置）→ 落本机战绩 + 上报全球排行榜
            lbAddLocal(__lbSession);
            lbSubmit(__lbSession);
            __lbSession = 0;
          }
        } catch (_) {}
      }
  ...
  function buildLevelModal() {
    ...
    var cheatRows =
      ...
      '    <div class="dzk-opt-row"><label>小球速度</label><input type="range" id="dzk-cheat-ballspeed" min="0.1" max="3" step="0.1" value="' + window.__dzkCheat.ballSpeedMul + '"><span class="dzk-opt-val" id="dzk-cheat-ballspeed-val">' + window.__dzkCheat.ballSpeedMul.toFixed(1) + '×</span></div>';
    ...
    wrap.innerHTML = [
      ...
      '        <div class="dzk-top-left">',
      '          <div class="dzk-endless-card" id="dzk-endless-card">',
      '            <div class="dzk-endless-info">',
      '              <div class="dzk-endless-title">♾️ 无尽模式 <span class="dzk-endless-floor-inline">层数 <b id="dzk-endless-floor">1</b></span></div>',
      '              <div class="dzk-endless-desc">球越打越快，挑战极限</div>',
      '              <div class="dzk-endless-fair" id="dzk-endless-fair" style="display:none">🔒 公平模式 · 参数已锁定为原始值</div>',
      '            </div>',
      ...
      '              <label>昵称<input type="text" id="dzk-lb-name" maxlength="16" placeholder="无名玩家"></label>',
      '              <label>Dreamlo 公开码（读取）<input type="text" id="dzk-lb-pub" placeholder="dreamlo.com 的 public code"></label>',
      '              <label>Dreamlo 私密码（上报）<input type="text" id="dzk-lb-priv" placeholder="dreamlo.com 的 private code"></label>',
      '              <div class="dzk-lb-hint">dreamlo.com 免费注册即得两个码；只填昵称 = 纯本机榜。留空两个码并保存 = 离线模式。</div>',
    ...
    if (lbGear && lbSettings) {
      ...
      var lbPubInput = wrap.querySelector('#dzk-lb-pub');
      var lbPrivInput = wrap.querySelector('#dzk-lb-priv');
      lbGear.addEventListener('click', function () {
        var c = lbCfg();
        lbNameInput.value = (c.name && String(c.name).trim()) || '';
        lbPubInput.value = c.pub || '';
        lbPrivInput.value = c.priv || '';
      ...
      wrap.querySelector('#dzk-lb-save').addEventListener('click', function () {
        lbSaveCfg({
          name: lbNameInput.value.trim(),
          pub: lbPubInput.value.trim(),
          priv: lbPrivInput.value.trim()
        });
        lbSettings.style.display = 'none';
