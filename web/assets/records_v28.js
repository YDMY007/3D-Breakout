...
(function () {
  ...
  // 关卡内部名（dat 文件）→ 中文显示名
  var LEVEL_NAMES_EN2 = {
    nm1: 'GATE', nm2: 'BARRIER', nm3: 'LOCK', nm4: 'CHAIN',
    nm5: 'ABYSS', nm6: 'MAW', nm7: 'DEVOUR', nm8: 'BONES',
    nm9: 'TOMB', nm10: 'VOID', nm11: 'EMBERS', nm12: 'SPIKE',
    nm13: 'SNARE', nm14: 'RUNE', nm15: 'HEX', nm16: 'EROSION',
    nm17: 'COLLAPSE', nm18: 'SILENCE', nm19: 'WASTELAND', nm20: 'FINALE'
  };
  var LEVEL_NAMES_CN = {
    ...
    nm1: '噩梦·门', nm2: '噩梦·闸', nm3: '噩梦·锁', nm4: '噩梦·链',
    nm5: '噩梦·狱', nm6: '噩梦·渊', nm7: '噩梦·噬', nm8: '噩梦·骸',
    nm9: '噩梦·墓', nm10: '噩梦·湮', nm11: '噩梦·烬', nm12: '噩梦·锥',
    nm13: '噩梦·阱', nm14: '噩梦·幂', nm15: '噩梦·咒', nm16: '噩梦·蚀',
    nm17: '噩梦·坍', nm18: '噩梦·寂', nm19: '噩梦·墟', nm20: '噩梦·终',
  ...
  // 昵称多层冗余写入：localStorage / sessionStorage / cookie 任一存活即可恢复，
  // 应对飞牛小窗清理、浏览器隐私策略等导致的"更新后重新分配昵称"
  function lbStoreName(name) {
    try { sessionStorage.setItem('dzk3d_name', name); } catch (_) {}
    try { document.cookie = 'dzk3d_name=' + encodeURIComponent(name) + ';path=/;max-age=31536000;samesite=lax'; } catch (_) {}
  }
  function lbRestoreName(c) {
    var rec = '';
    try {
      var m = document.cookie.match(/(?:^|;\s*)dzk3d_name=([^;]+)/);
      if (m) rec = decodeURIComponent(m[1]);
    } catch (_) {}
    if (!rec) { try { rec = sessionStorage.getItem('dzk3d_name') || ''; } catch (_) {} }
    if (rec) { c.name = rec; try { sessionStorage.setItem('dzk3d_name', rec); } catch (_) {} return rec; }
    return '';
  }
  function lbName() {
    var c = lbCfg();
    if (c.name && String(c.name).trim()) return String(c.name).trim();
    var rec = lbRestoreName(c);
    if (rec) return rec;
    // 确实没有任何历史昵称：随机生成并记住
    var n = lbRandomName();
    c.name = n;
  ...
  function lbSubmit(score, floors) {
    try {
      ...
      fetch(lbApi() + '/api/lb/submit', {
        method: 'POST',
        body: JSON.stringify({ name: myName, score: score, floors: floors })
      }).then(function (r) { return r.json(); }).then(function (j) {
        if (j && j.ok) {
          // 上报成功 → 记录本次上传用的昵称（改名时服务端据此迁移旧账户数据）
          try { localStorage.setItem('dzk3d_lb_lastsub', myName); } catch (_) {}
  ...
  function lbRenderRows(listEl, rows, mineName) {
    ...
    if (!rows || !rows.length) {
      ...
      rows.slice(0, 10).forEach(function (r, idx) {
        ...
        var fl = r.floors != null ? r.floors : r.floor;
        var floorTxt = fl ? fl + '层' : '';
        var d = new Date(Number(r.ts || r.date) || 0);
        var dateTxt = d.getTime() ? (d.getMonth() + 1) + '/' + d.getDate() : '';
        html += '<div class="dzk-lb-row' + me + '">' +
          '<span class="dzk-lb-rank">' + medal + '</span>' +
          '<span class="dzk-lb-name">' + String(r.name).replace(/[<>&]/g, '') + '</span>' +
          '<span class="dzk-lb-floor">' + floorTxt + '</span>' +
          '<span class="dzk-lb-score">' + r.score + ' 分</span>' +
          '<span class="dzk-lb-date">' + dateTxt + '</span></div>';
      });
    }
    listEl.innerHTML = html;
  }
  ...
  // ===== 游玩局数计数：普通关卡"通关"不计入游戏结束记录，单独包一层 finishLevel
  // （引擎每次普通关卡过关都会调用；无尽模式爬层也走这里，用 __dzkEndlessMode 排除）。
  // 游戏结束（生命耗尽）由引擎 dzk3d-record-updated 事件计数。两者相加 = 游玩局数。
  function hookFinishLevel() {
    try {
      var g = window.__webglInstance.$webgl.game;
      if (g && typeof g.finishLevel === 'function' && !g.finishLevel.__dzkHook) {
        var orig = g.finishLevel;
        g.finishLevel = function () {
          try {
            if (!window.__dzkEndlessMode) {
              var n = (Number(localStorage.getItem('dzk3d_runs_normal')) || 0) + 1;
              localStorage.setItem('dzk3d_runs_normal', String(n));
            }
          } catch (_) {}
          return orig.apply(this, arguments);
        };
        g.finishLevel.__dzkHook = true;
      }
    } catch (_) {}
  }
  if (!window.__dzkRunGoHooked) {
    window.__dzkRunGoHooked = true;
    window.addEventListener('dzk3d-record-updated', function () {
      try {
        var n = (Number(localStorage.getItem('dzk3d_runs_go')) || 0) + 1;
        localStorage.setItem('dzk3d_runs_go', String(n));
      } catch (_) {}
    });
  }
  ...
  function startCatchAssist() {
    setInterval(function () {
      try {
        ...
        // ===== 僵死检测：游戏进行中画面既无球也无方块（引擎卡死在运行态），
        // 连续约 2.5 秒（21 × 120ms）则自动触发 Home 键，弹出 继续/重新开始/关卡选择 面板。
        // 每个僵死回合只弹一次；条件消失（重新有球/方块/离开游戏）自动复位。
        try {
          var wInst = window.__webglInstance;
          var wGame = wInst && wInst.$webgl && wInst.$webgl.game;
          if (wGame && wGame.state && wGame.state.current && wGame.state.current.value) {
            var stId = wGame.state.current.value.id;
            var balls = 0;
            try { balls = wInst.$webgl.scene.balls.getActiveBallsCount(); } catch (_) { balls = -1; }
            var brk = -1;
            try { brk = wGame.blocks && wGame.blocks.breakableInstances ? wGame.blocks.breakableInstances.size : -1; } catch (_) { brk = -1; }
            var noModal = !document.querySelector('.dzk-level-modal.open') &&
                          !document.querySelector('.dzk-home-modal.open') &&
                          !document.querySelector('.dzk-records-modal.open');
            var stuck = stId === 'GAME_RUNNING' && balls === 0 && brk === 0 && noModal &&
                        !!document.querySelector('.page.page-game');
            if (stuck) {
              window.__dzkStuckTicks = (window.__dzkStuckTicks || 0) + 1;
            } else {
              window.__dzkStuckTicks = 0;
              window.__dzkAutoHomeShown = false;
            }
            if (stuck && window.__dzkStuckTicks >= 21 && !window.__dzkAutoHomeShown) {
              window.__dzkAutoHomeShown = true;
              var hb = document.querySelector('.dzk-home-btn');
              if (hb) hb.click(); // 与手动点 Home 完全一致：暂停 + 弹出 继续/重新开始/关卡选择
            }
          }
        } catch (_) {}
        ...
        hookFinishLevel(); // 游玩局数：普通关卡通关计数钩子（一次性安装）
      } catch (e) {}
    }, 120);
  }
  ...
  var LEVELS = [
    ...
    { name: "nm1", diff: 5 }, { name: "nm2", diff: 5 }, { name: "nm3", diff: 5 }, { name: "nm4", diff: 5 },
    { name: "nm5", diff: 5 }, { name: "nm6", diff: 5 }, { name: "nm7", diff: 5 }, { name: "nm8", diff: 5 },
    { name: "nm9", diff: 5 }, { name: "nm10", diff: 5 }, { name: "nm11", diff: 5 }, { name: "nm12", diff: 5 },
    { name: "nm13", diff: 5 }, { name: "nm14", diff: 5 }, { name: "nm15", diff: 5 }, { name: "nm16", diff: 5 },
    { name: "nm17", diff: 5 }, { name: "nm18", diff: 5 }, { name: "nm19", diff: 5 }, { name: "nm20", diff: 5 },
  ...
  var DIFF_NAMES = ['教学', '简单', '普通', '困难', '专家', '噩梦'];
  var DIFF_COLORS = ['#4CD97B', '#51AFFF', '#FFB84D', '#FF7A51', '#FF4D6A', '#B04DFF'];
  var DIFF_ICONS  = ['🌱', '⭐', '🔥', '⚡', '💀', '☠️'];
  var levelModal = null;
  ...
  function buildLevelModal() {
    ...
    wrap.setAttribute('data-bypass-touch', ''); // 引擎对带此标记的 target 不做 touchstart preventDefault,手机端弹窗内可正常滑动
    ...
    if (lbGear && lbSettings) {
      ...
      wrap.querySelector('#dzk-lb-save').addEventListener('click', async function () {
        var oldName = lbName();
        var newName = lbNameInput.value.trim() || lbRandomName();
        var prevSub = '';
        try { prevSub = localStorage.getItem('dzk3d_lb_lastsub') || ''; } catch (_) {}
        var week = 7 * 24 * 3600 * 1000;
        var lastRen = 0;
        try { lastRen = Number(localStorage.getItem('dzk3d_lb_renamets')) || 0; } catch (_) {}
        // 本机曾以旧昵称上传过成绩且这次改了名 → 让服务端把旧账户数据迁移到新名下（7 天一次）
        if (prevSub && prevSub !== newName) {
          if (Date.now() - lastRen < week) {
            alert('改名太频繁：一个星期只能改一次，还需等待约 ' + Math.ceil((week - (Date.now() - lastRen)) / 86400000) + ' 天');
            return;
          }
          try {
            var rr = await fetch(lbApi() + '/api/lb/admin/rename', {
              method: 'POST',
              body: JSON.stringify({ old: prevSub, new: newName })
            });
            var rj = null; try { rj = await rr.json(); } catch (_) {}
            if (rj && rj.ok) {
              try { localStorage.setItem('dzk3d_lb_renamets', String(Date.now())); } catch (_) {}
            } else if (rj && rj.error === 'rename cooldown') {
              alert('改名太频繁：一个星期只能改一次，还需等待约 ' + Math.ceil((rj.remainMs || 0) / 86400000) + ' 天');
              return;
            }
            // not found = 旧昵称没有服务器数据，无需迁移，直接改本机昵称
          } catch (_) {}
        }
        lbSaveCfg({
          name: newName,
  ...
  function renderStatsCard() {
    ...
    var runsGo = Number(localStorage.getItem('dzk3d_runs_go'));
    if (isNaN(runsGo)) {
      // 首次启用计数：以既有游戏结束记录打底，历史不归零
      runsGo = recs.length;
      try { localStorage.setItem('dzk3d_runs_go', String(runsGo)); } catch (_) {}
    }
    var runsNormal = Number(localStorage.getItem('dzk3d_runs_normal')) || 0;
    var items = [
      ['🏆 历史最高', best > 0 ? fmt(best) : '暂无'],
      ['🎮 游玩局数', String(runsGo + runsNormal)],
  ...
  function renderLevelModal() {
    ...
    for (var d = 0; d < 6; d++) {
      if (!groups[d].length) continue;
      // 折叠状态记忆：只展开一个组——上次玩过的难度；无记录则展开第一个(教学)。
      // 手动展开/收起过的组以记忆为准（dzk3d_fold_user='1' 表示用户手动管过折叠）
      var foldKey = 'dzk3d_fold_d' + d;
      var folded;
      var lastDiff = null;
      try {
        var ll = localStorage.getItem('dzk3d_last_level');
        if (ll) {
          var rec = LEVELS.filter(function (x) { return x.name === ll; })[0];
          lastDiff = rec ? rec.diff : null;
        }
      } catch (_) {}
      try {
        var saved = localStorage.getItem(foldKey);
        var userTouched = localStorage.getItem('dzk3d_fold_user') === '1';
        if (userTouched && saved !== null) {
          folded = saved === '1';            // 用户手动管过：完全按记忆
        } else {
          // 自动模式:展开目标组(上次难度或第一个),其余收起
          var target = (lastDiff != null) ? lastDiff : 0;
          folded = d !== target;
        }
      } catch (_) { folded = d !== (lastDiff != null ? lastDiff : 0); }
      html += '<div class="dzk-level-group' + (folded ? ' dzk-folded' : '') + '" data-diff="' + d + '">' +
        '<button type="button" class="dzk-level-diff" style="border-color:' + DIFF_COLORS[d] + ';color:' + DIFF_COLORS[d] + '" aria-expanded="' + (folded ? 'false' : 'true') + '">' +
        '<span class="dzk-level-diff-icon" aria-hidden="true">' + DIFF_ICONS[d] + '</span>' +
        DIFF_NAMES[d] +
        '<span class="dzk-level-diff-count">' + groups[d].length + ' 关</span>' +
        '<span class="dzk-level-fold-arrow" aria-hidden="true">▾</span></button>' +
        '<div class="dzk-level-list">';
      groups[d].forEach(function (lv, idx) {
        var cn = LEVEL_NAMES_CN[lv.name] || lv.name;
        var en = LEVEL_NAMES_EN2[lv.name] || lv.name;
        var last = (localStorage.getItem('dzk3d_last_level') === lv.name) ? ' dzk-level-btn-last' : '';
        html += '<button class="dzk-level-btn' + last + '" data-level="' + lv.name + '" data-name-cn="' + cn + '" data-name-en="' + en + '">' +
          '<span class="dzk-level-num">' + (idx + 1) + '</span>' +
          '<span class="dzk-level-name">' + cn + '</span>' +
          '<span class="dzk-level-en">' + en + '</span>' +
          '</button>';
      });
      html += '</div></div>';
    }
    groupsEl.innerHTML = html;
    // 分组折叠：标题点击 收起/展开 + 记忆到 localStorage；点关卡时若组被折叠先自动展开
    if (!groupsEl.__dzkFoldBound) {
      groupsEl.__dzkFoldBound = true;
    groupsEl.addEventListener('click', function (ev) {
      var head = ev.target.closest('.dzk-level-diff');
      if (head && head.tagName === 'BUTTON') {
        var grp = head.parentElement;
        var folded = grp.classList.toggle('dzk-folded');
        head.setAttribute('aria-expanded', folded ? 'false' : 'true');
        try {
          localStorage.setItem('dzk3d_fold_d' + grp.getAttribute('data-diff'), folded ? '1' : '0');
          localStorage.setItem('dzk3d_fold_user', '1');
        } catch (_) {}
      }
    });
    }
    ...
    groupsEl.querySelectorAll('.dzk-level-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var grp = btn.closest('.dzk-level-group');
        if (grp && grp.classList.contains('dzk-folded')) {
          grp.classList.remove('dzk-folded');
          var h = grp.querySelector('.dzk-level-diff');
          if (h) h.setAttribute('aria-expanded', 'true');
          try { localStorage.setItem('dzk3d_fold_d' + grp.getAttribute('data-diff'), '0'); } catch (_) {}
        }
