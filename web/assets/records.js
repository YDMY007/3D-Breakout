    ...
    wrap.addEventListener('click', function (e) {
      ...
      if (act) {
        if (act === 'play') {
          ...
          wrap.classList.remove('open');
          if (window.__dzkCheat && window.__dzkCheat.autoStick) {
            // 自动吸附模式：绕道 Home 会被 vendor 旁路直接弹回 Game，暂停不解除——就地打开选关
            window.dispatchEvent(new Event('dzk3d-request-level-select'));
          } else {
            window.dispatchEvent(new Event('dzk3d-game-level-select'));
          }
        }
      }
    });
