...
(function () {
  ...
  function openModal() {
      ...
      pausedByTrophy = isGameRoute();
      if (pausedByTrophy) window.dispatchEvent(new Event('dzk3d-game-pause'));
    ...
    function closeModal() {
      ...
      if (pausedByTrophy) {
        pausedByTrophy = false;
        try {
          if (isGameRoute()) window.dispatchEvent(new Event('dzk3d-game-play'));
        } catch (e) {}
      }
    }
  ...
  function lbSubmit(floor) {
    try {
      ...
      fetch(lbApi() + '/api/lb/submit', {
        method: 'POST',
        body: JSON.stringify({ name: lbName(), floor: floor })
      }).then(function (r) { return r.json(); }).then(function (j) {
        if (j && j.ok && j.rank) {
          try { localStorage.setItem('dzk3d_lb_lastrank', String(j.rank)); } catch (_) {}
        }
      }).catch(function () {});
    } catch (_) {}
  }
        ...
        try {
          ...
          if (Array.isArray(players)) {
            rows = players.map(function (p) {
              return { name: String(p.name || '?'), floor: Number(p.floor != null ? p.floor : p.score) || 0 };
            });
      ...
      set: function (v) {
        ...
        try {
          if (v > 1) {
            ...
            // 单局结束（层数被重置）：本机战绩每次都记；**只有打破个人纪录才上传**
            lbAddLocal(__lbSession);
            if (__lbSession > __lbPrevBest) {
              lbSubmit(__lbSession);
              try { localStorage.setItem('dzk3d_lb_lastsubmit', String(__lbSession)); } catch (_) {}
            }
  ...
  function buildLevelModal() {
    ...
    wrap.innerHTML = [
      ...
      '            <div class="dzk-lb-hint">成绩 = 无尽层数，打破个人纪录自动上传；数据上报至 690075.xyz（Cloudflare Workers）。<a class="dzk-lb-privacy-toggle" id="dzk-lb-privacy-toggle">隐私条款</a></div>',
      '            <div class="dzk-lb-privacy" id="dzk-lb-privacy" style="display:none">收集内容：仅昵称（首次进入自动生成，可修改）与无尽模式到达层数、提交时间。<br>用途：仅用于全球排行榜的展示与排序，不作其他用途。<br>不收集：邮箱、手机号、账号密码等任何身份信息。<br>IP 处理：为防刷屏，服务器以 IP 做短期（约 10 分钟）提交频率限制，不公开显示、不与榜单长期关联。<br>存储位置：Cloudflare Workers KV（全球边缘节点）；本机最高分与战绩存于浏览器本地，清除浏览器数据即删除。<br>想更换榜单身份：改昵称即可；需要删除榜单条目请联系站点管理员。</div>',
