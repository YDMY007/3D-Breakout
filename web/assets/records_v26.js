...
(function () {
  ...
  function renderLevelModal() {
    ...
    for (var d = 0; d < 6; d++) {
      if (!groups[d].length) continue;
      // 折叠状态记忆：默认展开"上次玩过"的难度组，其余收起（首次打开全部收起只留教学）
      var foldKey = 'dzk3d_fold_d' + d;
      var folded;
      try {
        var saved = localStorage.getItem(foldKey);
        folded = saved === null ? (d !== 0 && localStorage.getItem('dzk3d_last_level') &&
          (LEVELS.filter(function (x) { return x.name === localStorage.getItem('dzk3d_last_level'); })[0] || {}).diff !== d)
          : saved === '1';
        if (saved === null && d === 0) folded = false;
      } catch (_) { folded = d !== 0; }
    ...
    groupsEl.addEventListener('click', function (ev) {
      ...
      if (head && head.tagName === 'BUTTON') {
        ...
        try { localStorage.setItem('dzk3d_fold_d' + grp.getAttribute('data-diff'), folded ? '1' : '0'); } catch (_) {}
      }
    });
    }
